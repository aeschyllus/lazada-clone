...
it("should render link type", () => {
  ...
  expect(container.firstElementChild?.className).toBe(
    "transition ease-in-out outline-none hover:text-orange-500 focus:text-orange-500"
  );
...
it("should render button type", () => {
  ...
  expect(getByRole("link", { name: "test" })).toBeDefined;
  expect(container.firstElementChild?.className).toBe(
    "transition ease-in-out outline-none bg-orange-500 text-white rounded-md px-3 py-2 hover:bg-orange-600 focus:bg-orange-600"
  );
